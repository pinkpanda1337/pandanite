...
def compute_difficulty(current_difficulty: int, elapsed_time: int, expected_time: int) -> int:
    ...
    if elapsed_time > expected_time:
        ...
        while new_difficulty > MIN_DIFFICULTY:
            if abs(elapsed_time // k - expected_time) > abs(elapsed_time // last_k - expected_time):
                break
            new_difficulty -= 1
            last_k = k
            k *= 2
